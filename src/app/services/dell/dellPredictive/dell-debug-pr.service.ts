...
export class DellDebugPrService {
  constructor(
  ...
  handleDellDebugPRActions(action: any, instance: any, actionService: any) {
    switch (action.methodType) {
      ...
      case 'checkForReplaceTask':
        ...
        break;
      case 'updatePredictionData':
        this.updatePredictionData(action, actionService)
        break;
      case 'handle2ndVisit':
        this.handle2ndVisit(action, instance, actionService)
        break;
      case 'reqNotesPopUpOnTimeOUt':
        this.reqNotesPopUpOnTimeOUt(instance, actionService)
        break;
        case 'resetAndUpdateForPredictiveIcon':
          this.resetAndUpdateForPredictiveIcon(action,instance, actionService)
          break;
  ...
  handleDellProcessActions(action: any, instance: any, actionService: any) {
    ...
    let isPredictionLoop = false;
    let processActions = [];
    let defectClass = "top-margin-before"
    if (action.config.targetId == undefined) {
      action.config["targetId"] = "pageUUID";
      action.config["containerId"] = "prebodysectiontwo";
      action.config["isPagePosition"] = true;
      defectClass = "top-margin-taskPanel"
    }
    if (action.isNewPredictiveTask) {
      action.config["isPredictionLoop"] = true
    }
    ...
    processActions = [
      ...
      },
      {
        ...
        "config": {
          "targetId": action.config.targetId,
          "containerId": action.config.containerId ? action.config.containerId : "expansionPanelContenttaskpanel",
          "data": {
            "ctype": "taskPanel",
            "uuid": action.config.index ? "dellDebugTaskPanelUUID" + action.config.index : "dellDebugTaskPanelUUID",
            "title": "",
            "header": {
            ...
            "actions": [
              ...
              },
              {
                ...
                "eventSource": "click",
                "config": {
                  "key": action.config.index ? "dellDebugTaskPanelUUID" + action.config.index : "dellDebugTaskPanelUUID"
                }
              },
              {
                "type": "context",
            ...
            "footer": [
              {
                ...
                "actions": [
                  ...
                  {
                    "type": "handleDellDebugPRActions",
                    "methodType": "resetAndUpdateForPredictiveIcon",
                    "config": action,
                    "eventSource": "click"
                  }
                ]
              },
              {
                ...
                "parentuuid": "replaceTaskUUID",
                "parentUUID": action.config.targetId,
                "visibility": true,
                "disabled": true,
                "type": "submit",
                "tooltip": "",
  ...
  private _disableOrEnableAllIcons(action: any, instance: any, actionService: any) {
    ...
    let dynamicIconValue = action.config.targetId;
    let addData = this.contextService.addToContext("dynamicIconValue", dynamicIconValue);
    let dynamicList = this.contextService.getDataByKey(dynamicIconValue + "ref");
    let dynamaicIconList = [];
    if (!!dynamicList?.instance?.toolBarItems) {
      dynamaicIconList = dynamicList.instance.toolBarItems[0].items;
      dynamaicIconList.forEach((dynamicIcon) => {
        if (action.config.currentProcess !== undefined &&
          !dynamicIcon.uuid.includes(action.config.currentProcess)) {
          if (action.config.isDisable !== undefined) {
            actionService.handleAction({
              "type": "updateComponent",
              "config": {
                "key": dynamicIcon.uuid ? dynamicIcon.uuid : dynamicIcon,
                "properties": {
      ...
      dynamaicIconList.forEach((currentIcon) => {
        if (action.config.currentProcess !== undefined &&
  ...
  returnDefectGroupHooks(action: any) {
    ...
    if(action.config.Defectgroup){
       selectedDefectGroup =this.contextService.getDataByString(action.config.Defectgroup);
    }
    return [
      {
        "type": "condition",
            ...
            "actions": [
              ...
              },
              {
                ...
                "config": {
                  "key": "dellDebugDefectCodeGroupUUID",
                  "dropDownProperties": {
                    "options":  action.config.DefectGroupOptions
                  }
                }
              },
              {
                ...
                "config": {
                  "key": "dellDebugDefectCodeGroupUUID",
                  "defaultValue": action.config.Defectgroup
                }
              },
              {
                "type": "context",
  ...
  returnDefectHooks(action: any) {
    let selectedDefectCode,selectedDefectCodeobj={"description":""},isSelectedDefect=null;
    if(action.config.DefectCode){
      isSelectedDefect=true
     selectedDefectCode =this.contextService.getDataByString(action.config.DefectCode);
     selectedDefectCodeobj= action.config.DefectCodeOptions && action.config.DefectCodeOptions.find(element => element.defectCode == selectedDefectCode );
    }
    return [
      {
        "type": "condition",
            ...
            "actions": [
              ...
              },
              {
                ...
                "config": {
                  "key": "dellDebugDefectUUID",
                  "dropDownProperties": {
                    "options":  action.config.DefectCodeOptions
                  }
                }
              },
              {
                ...
                "config": {
                  "key": "dellDebugDefectUUID",
                  "defaultValue": action.config.DefectCode
                }
              },
              {
                "type": "context",
              ...
              },
              {
                ...
                "config": {
                  "requestMethod": "add",
                  "key": "dellDebug" + action.config.processType + "DefectName",
                  "data": selectedDefectCodeobj.description
                },
                "eventSource": "click"
              },
              {
 ...
 */
  ...
  private _transactionsOnClickOfComplete(action: any, instance: any, actionService: any) {
    ...
    action.config["isPredictionLoop"] = action.config.isPredictionLoop;
    action.config["index"] = action.config.index;
    action.config["DefectGroup"] =this.contextService.getDataByString( "#dellDebug" + action.config.processType + "DefectGroupName");    
    let transactions = [];

    if(action.config.isPredictionLoop){
      transactions.push({
  ...
  private _onClickOfCompleteButton(action: any, instance: any, actionService: any) {
    ...
    if(!!action?.config?.addItemToReqList){
      addItemToReqList = action.config.addItemToReqList;
    }
    let targetId;
    let containerId;
    let defaultStataus = "Diagnosis";
    let titleClass = "top-margin";
    if (!action.config.isLooperControlTask) {
      action = action.config.action;
    } else {
    ...
    let AcceptedPredictionTaskPanels = this.contextService.getDataByKey("AcceptedPredictionTaskPanels");
    // if (action.isNonPredictiveTask !== undefined && !action.isNonPredictiveTask) {
    //   targetId = instance.parentUUID;
    //   //   containerId = "expansionPanelContenttaskpanel";
    //   defaultStataus = "Prediction";
    //   // titleClass = "top-margin-before"
    // } else {
    //   targetId = instance.parentUUID;
    // }
    if(action.config.taskPanelData.isPrediction){
      defaultStataus = "Prediction";
    }
    if (!!action?.isNonPredictiveTask && !action.isNonPredictiveTask) {
      targetId = instance.parentUUID;
      containerId = "expansionPanelContenttaskpanel";
      titleClass = "top-margin-before"
    } else if (action.config.isPagePosition !== undefined && action.config.isPagePosition) {
      ///this condition is used for non predictive taskpanels
      targetId = "pageUUID";
      titleClass = "top-margin";
      containerId = "prebodysection";
    }
    else if (action.config.targetId && action.config.targetId !== undefined) {
      ///this condition is used for non predictive taskpanels
    ...
    if (newHPFAHistory && newHPFAHistory !== "" && newHPFAHistory.length > 0 && !addItemToReqList) {
    ...
    actions = [
      ...
      },
      {
        ...
        "config": {
          ...
          "data": {
            "ctype": "taskPanel",
            "uuid": action.config.taskPanelData.childUUID ? action.config.taskPanelData.childUUID : "dellDebugCreatedTaskPanelUUID",
            "isyellowBorder": true,
            "uniqueUUID": action.config.taskPanelData.childUUID ? false : true,
            "updateUUID": true,
            "title": "",
  ...
  private _createdTaskHooks(action: any, addItemToReqList) {
    let hooks = [];
    let getDellDebugStockQty = action.config.isLooperControlTask && !!action.config.getDellDebugStockQty ? action.config.getDellDebugStockQty : "#getDellDebugStockQty";
    let taskPanelDataQty = action.config.taskPanelData.partDetails;
    if(taskPanelDataQty.length){
      action.config.taskPanelData.partDetails=taskPanelDataQty
      getDellDebugStockQty=taskPanelDataQty
    }
    if (action.config.processType === "replace") {
      hooks = [
        {
              ...
              "actions": [
                {
                  ...
                  "config": {
                    ...
                    "sourceData": {
                      ...
                      "parentUUID": action.config.taskPanelData.childUUID ? action.config.taskPanelData.childUUID : "#createdComponentUUID"
                    }
                  },
                  "hookType": "afterInit"
                },
                ...
                },
                {
                  ...
                  "config": {
                    ...
                    "sourceData": {
                      "parentUUID": action.config.taskPanelData.childUUID ? action.config.taskPanelData.childUUID : "#@",
        ...
        },
        {
          ...
          "methodType": "updateStockQty",
          "config": {
            "data": action.config.taskPanelData.partDetails,
            "keyToUpdate": "dellDebugStockQty#@",
            "isDisable": false
          },
          "hookType": "afterInit"
    ...
    if (!!action.config.processType && action.config.processType !== "predictive") {
      hooks.push({
        "type": "handleDellDebugPRActions",
        "methodType": "disableOrEnableAllIcons",
        "hookType": "afterInit",
    ...
    if(!!action.config.isLooperControlTask && action.config.isLooperControlTask && !! action.config.isLastItem &&  action.config.isLastItem && action.config.processType==="replace"){
      hooks.push({
        "type": "triggerreqclick",
        "hookType": "afterInit",
        "config": {
...
*/
  private _stocktoDisplay(stockToDisplay: any) {
    let stock=stockToDisplay;
    if(typeof stockToDisplay === "string" && stockToDisplay.startsWith("#")){
      stock = this.contextService.getDataByString(stockToDisplay);
    }
  ...
  private _getFooter(action: any, instance: any, actionService: any, isTaskcompleted: boolean) {
    ...
    let deleteButton = {
      ...
      "isIcon": true,
      "type": "submit",
      "actions": this.deleteButtonActions(action)
    };
   ...
   */
  deleteButtonActions(action: any, isBulkDelete?) {
    let uuid = action.config.taskPanelData.childUUID ? action.config.taskPanelData.childUUID : "#@";
    let actions = [];
    actions = [
      {
              ...
              "defectCodeChange": [
                {
                  "defectCode": action.config.taskPanelData.defect,
                  "operation": "Delete",
                  "occurrence": !!isBulkDelete ? "1" : "#currentOccurenceData" + uuid + ".defectCodeOccurence"
                }
              ]
            }
          },
          ...
          "actions": [
            ...
            },
            {
              ...
              "config": {
                "target": "#occurenceList",
                "key": uuid,
                "currentTaskData": "#currentOccurenceData" + uuid
              },
              "eventSource": "click"
            },
            {
              "type": "handleDellDebugPRActions",
              "methodType": "updatePredictionData",
              "config": {
                "uuid": uuid,
                "defectCode": action.config.taskPanelData.defect,
              },
                  ...
                  "actions": [
                    {
                      ...
                      "config": {
                        ...
                        "PullValue": uuid,
                        "key": !!isBulkDelete && isBulkDelete ? uuid : "parentUUID",
                        "property": "flexFields",
                        "splice": true,
    ...
    if (action.config.arrayData.startsWith('#')) {
      ...
      if (action.config.key !== undefined) {
        ...
        if (action.config.splice !== undefined && action.config.splice) {
          ...
          if (action.config.deleteTableData !== undefined && requisitionList) {
  ...
  updateStockQty(action, instance, actionService) {
    ...
    if(typeof action.config.data === "string" && action.config.data.startsWith("#")){
 ...
 */
  ...
  private _createLooperControlPanels(action: any, instance: any, actionService: any) {
    ...
    if (!action.config.isNonPredictiveTask) {
      looperControlData = this.contextService.getDataByKey("newPredictionData");
      looperControlData = looperControlData.childList
    }
        ...
        actionCodes && actionCodes.forEach((element) => {
          if (element.actonCodesAbbreviation === item.actionCode) {
            if (!!predictiveData && predictiveData.length > 0) {
              predictiveData.map((currentPredictiveItem) => {
                if (currentPredictiveItem.defectCode === item.defectCode) {
                  isPredictive = true;
                }
              })
            }
    ...
    //   {
    //     "type": "handleDellDebugPredictionActions",
    //     "methodType": "onClickofCompleteButInPopup",
    ...
    itemsAdded && itemsAdded.forEach((item, index) => {
      ...
      item["isNonPredictiveTask"] = action.config.isNonPredictiveTask;
  ...
  afterCombineLatest(itemsAdded, actionService, instance) {
    ...
    actionService.handleAction({
      "type": "handleDellDebugPRActions",
      "methodType": "onClickOfCompleteButton",
      "isNonPredictiveTask": item.isNonPredictiveTask,
      "config": {
        "svgIcon": "replace",
        "processType": "replace",
        "taskPanelData": taskPanelData,
  ...
  /// Opens the requisition pop up and saves this as req notes
  reqNotesPopUpOnTimeOUt(instance, actionService){
        ...
        "responseDependents": {
          "onSuccess": {
            "actions": [
              {
                "type": "dialog",
                "uuid": "dellDebugReqNotesDialogUUID",
                "config": {
                  ...
                  "items": [
                    {
                      ...
                      "required": !!location && location === "1244" ? true : false,
                      "disabled": false,
                      "value": "",
                      "actions": []
                    }
                  ...
                  "footer": [
                    ...
                    },
                    {
                      ...
                      "color": "primary",
                      "text": "Done",
                      "uuid": "dellDebugDoneButtonUUID",
                      "name": "dellDebugDoneButton",
                      "dialogButton": true,
                      "visibility": true,
                      "disabled": !!location && location === "1244" ? true : false,
                      "type": "submit",
                      "closeType": "success",
                      "hooks": [],
                      "validations": [],
                  ...
                  "onSuccess": {
                    "actions": [
                      ...
                      },
                      {
                        ...
                        "config": {
                          "requestMethod": "add",
                          "key": "dellDebugreplaceReqNotes",
                          "data": "#reqPopUpFormData.dellDebugReqNotes"
                        },
                        "eventSource": "click"
                      },
                      {
                        "type": "handleDellDebugPRActions",
                        "methodType": "reqSaveButtonActions",
                        "eventSource": "click"
                      }
                    ]
                  },
    ...
    this.executeActions(reqNotesPopUp, instance, actionService);
  }
  ...
  checkForNewlyAddedItem(action, instance, actionService) {
    ...
    if (action.config.addTaskPanel === undefined) {
      actionService.handleAction({
        "type": "handleDellDebugPRActions",
        "methodType": "onClickOfCompleteButton",
        "config": {
  ...
  updatePredictionData(action, actionService){
    let uuid = action.config.uuid;
    let defectCode = action.config.defectCode;
    let predictiveData = this.contextService.getDataByString("#predictionData");
    if(defectCode && defectCode.startsWith("#")){
      defectCode = this.contextService.getDataByString(defectCode);
    }
    predictiveData && predictiveData.map((x) => {
      if(x.defectCode === defectCode){
        x.childList && x.childList.map((child, index) => {
          if(child.childUUID === uuid){
            x.childList.splice(index, 1);
          }
        })
      }
    })
  ...
  resetAndUpdateForPredictiveIcon(action: any, instance: any, actionService: any) {
    let selectedDefectGroup;
    if(action.config.config.Defectgroup){
       selectedDefectGroup =this.contextService.getDataByString(action.config.config.Defectgroup);
    }
    let selectedDefectCode,selectedDefectCodeobj={"description":""},isSelectedDefect=null;
    if(action.config.config.DefectCode){
      isSelectedDefect=true
     selectedDefectCode =this.contextService.getDataByString(action.config.config.DefectCode);
     selectedDefectCodeobj= action.config.config.DefectCodeOptions && action.config.config.DefectCodeOptions.find(element => element.defectCode == selectedDefectCode );
    }
    let actions=[
      {
      "type": "condition",
      "config": {
        "operation": "isValid",
        "lhs": action.config.config.Defectgroup
      },
      "hookType": "afterInit",
      "responseDependents": {
        "onSuccess": {
          "actions": [
            ...
            },
            {
              ...
              "config": {
                "key": "dellDebugDefectCodeGroupUUID",
                "dropDownProperties": {
                  "options":  action.config.config.DefectGroupOptions
                }
              }
            },
            {
              ...
              "config": {
                "key": "dellDebugDefectCodeGroupUUID",
                "defaultValue": action.config.config.Defectgroup
              }
            },
            {
              "type": "context",
              "config": {
                "requestMethod": "add",
                "key": "dellDebug" + action.config.config.processType + "DefectGroupName",
                "data": selectedDefectGroup
              },
              "eventSource": "click"
            },
          ...
          "actions": [
            ...
            },
            {
              ...
              "config": {
                "key": "dellDebugDefectUUID",
                "dropDownProperties": {
                  "options":  action.config.config.DefectCodeOptions
                }
              }
            },
            {
              ...
              "config": {
                "key": "dellDebugDefectUUID",
                "defaultValue": action.config.config.DefectCode
              }
            },
            {
              "type": "context",
              "config": {
                "requestMethod": "add",
                "key": "dellDebug" + action.config.config.processType + "DC",
                "data": selectedDefectCode
              },
              "eventSource": "click"
            },
            {
              ...
              "config": {
                "requestMethod": "add",
                "key": "dellDebug" + action.config.config.processType + "DefectName",
                "data": selectedDefectCodeobj.description
              },
              "eventSource": "click"
            },
